...
public enum RecordField {
  ...
  // Batch Control
  BC_SERVICE_CLASS_CODE(RecordType.BATCH_CONTROL, 2, 4),
  BC_ENTRY_AND_ADDENDA_COUNT(RecordType.BATCH_CONTROL, 5, 10),
  BC_ENTRY_HASH(RecordType.BATCH_CONTROL, 11, 20),
  BC_DEBIT_AMOUNT(RecordType.BATCH_CONTROL, 21, 32),
  BC_CREDIT_AMOUNT(RecordType.BATCH_CONTROL, 33, 44),
  BC_COMPANY_IDENTIFICATION(RecordType.BATCH_CONTROL, 45, 54),
  BC_MESSAGE_AUTHENTICATION_CODE(RecordType.BATCH_CONTROL, 55, 73),
  BC_RESERVED(RecordType.BATCH_CONTROL, 74, 79),
  BC_ORIGINATING_DFI_ID(RecordType.BATCH_CONTROL, 80, 87),
  BC_BATCH_NUMBER(RecordType.BATCH_CONTROL, 88, 94),

  // File Control
  FC_BATCH_COUNT(RecordType.FILE_CONTROL, 2, 7),
  FC_BLOCK_COUNT(RecordType.FILE_CONTROL, 8, 13),
  FC_ENTRY_AND_ADDENDA_COUNT(RecordType.FILE_CONTROL, 14, 21),
  FC_ENTRY_HASH(RecordType.FILE_CONTROL, 22, 31),
  FC_DEBIT_AMOUNT(RecordType.FILE_CONTROL, 32, 43),
  FC_CREDIT_AMOUNT(RecordType.FILE_CONTROL, 44, 55),
  FC_RESERVED(RecordType.FILE_CONTROL, 56, 94);
